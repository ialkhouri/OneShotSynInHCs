...
####################################################################################
################################ same coarser and different finer lable fixing
####################################################################################
target_finer_save = pickle.load(open("/home/ismail/pycharmProjects/SSLTL_project/RL_adv_attacks_LP/target_finer_save.p","rb"))
# target_finer_save = []
#
...
#
#     predicted_coarser_of_clean_image = np.argmax(trained_model_coarser(X_test[idx].reshape(1,784,1)).numpy())
#
#
#     ### the choice is made here as: same coarser, then select any finer randomly other than the predicted one
#     if predicted_coarser_of_clean_image == 0:
#         trained_model_finer = finer_1_model_1d
#         nnumber_of_classes_finer = 3
#
#         predicted_finer_of_clean_image = np.argmax(trained_model_finer(X_test[idx].reshape(1, 784, 1)).numpy())
#
#         temp = np.setdiff1d(np.array(range(nnumber_of_classes_finer)), predicted_finer_of_clean_image)
#
#         temp_2 = np.random.choice(temp)
#
#         target_finer_save.append(temp_2)
#
#     if predicted_coarser_of_clean_image == 1:
...
#
#
#     if predicted_coarser_of_clean_image == 2:
...
#         target_finer_save.append(temp_2)
#
# pickle.dump(target_finer_save, open("target_finer_save.p", "wb"))
####################################################################################
####################################################################################
####################################################################################
...
for idx in range(number_of_observations):
    ...
    if predicted_coarser_of_clean_image == 0:
        trained_model_finer = finer_1_model_1d
        nnumber_of_classes_finer = 3
        target_lbl_finer = target_finer_save[idx]
    if predicted_coarser_of_clean_image == 1:
        trained_model_finer = finer_2_model_1d
        nnumber_of_classes_finer = 4
        target_lbl_finer = target_finer_save[idx]
    ...
    target_class_coarser = predicted_coarser_of_clean_image
    ...
    #### #### passing criteria: prediction_coarser(X_BOSS) = prediction_coarser(X) AND
    #### #### prediction_finer(X_BOSS) = target_finer
    if np.argmax(trained_model_coarser(fake_image.reshape(1, 784, 1)).numpy()[0]) == np.argmax(trained_model_coarser(X_desired.reshape(1, 784, 1)).numpy()[0]) and np.argmax(trained_model_finer(fake_image.reshape(1, 784, 1)).numpy()[0]) == target_lbl_finer:
        succ = succ + 1

    #### outer loop logger:
    print('[index,succ] = ', [idx, succ], ' - [prediction C,F] = ', [np.argmax(trained_model_coarser(fake_image.reshape(1, 784, 1)).numpy()[0]), predicted_coarser_of_clean_image], [np.argmax(trained_model_finer(fake_image.reshape(1, 784, 1)).numpy()[0]),target_lbl_finer],
