...
for idx in range(number_of_observations):
    ...
    if np.argmax(trained_model(X_desired.reshape(1, 784, 1)).numpy()[0]) == np.argmax(trained_model(fake_image.reshape(1, 784, 1)).numpy()[0]):
        succ = succ+1


    #### outer loop logger:
    print('[index,succ] = ',[idx,succ],' - [confidence] = ',[confidence_score,confidence_score_BOSS],' - [SSIM,JS] = ',[D_ssim_images,D_JS])

    D_ssim_images_save.append(D_ssim_images)
    D_JS_save.append(D_JS)
    con_save.append(confidence_score_BOSS)
    stop = timeit.default_timer()
    run_time_save.append(stop - start)

print('OVERALL PERFORMANCE: ', 'succ = ' , 100*(succ/number_of_observations) , 'AVG_SSIM_JS = ',[np.mean(D_ssim_images_save),np.mean(D_JS_save)],"AVG_confidence_score", [np.mean(con_save)] , ' AVG runtime = ',np.mean(run_time_save))
